...
class UploadController extends Controller
{
    ...
    public function finance_order_upload()
    {
        header("content-type:text/html;charset=utf-8");

        $payment_type = $_POST['payment_type'];
        if (!in_array($payment_type, ['amazon_us', 'amazon_uk', 'amazon_de', 'amazon_es', 'amazon_fr', 'amazon_it', 'wayfair', 'walmart', 'shein', 'shein_semi_managed', 'temu', 'temu_detail', 'ebay', 'wildberries', 'tiktok'])) {
    ...
    public function warehouse_wfs_upload()
    {
        header("content-type:text/html;charset=utf-8");

        $user_account = $_POST['user_account'];
        if (empty($user_account)) {
            echo json_encode(['code' => 0, 'msg' => '请先选择你要上传的店铺']);
            exit();
        }
