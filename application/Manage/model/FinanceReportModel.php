...
FROM
	...
	SELECT
		...
		NULL AS sale_amount,
		ROUND( ( product_sales + shipping_credits + gift_wrap_credits + regulatory_fee + promotional_rebates ) * c.percent * c.qty, 7 ) refund_amount,
		NULL AS sale_selling_fees,
		ROUND( selling_fees * c.percent * c.qty, 7 ) refund_selling_fees,
		ROUND( fba_fees * c.percent * c.qty, 7 ) fba_fees,
		NULL AS ddp,
		NULL AS tail 
	FROM
	...
	FROM
		...
		FROM
			...
			SELECT
				...
				NULL AS fba_sale_amount,
				NULL AS fba_sale_tax,
				ROUND( ( product_sales + shipping_credits + gift_wrap_credits + regulatory_fee + promotional_rebates ) * c.percent * c.qty, 7 ) fba_refund_amount,
				NULL AS fba_sale_selling_fees,
				ROUND( selling_fees * c.percent * c.qty, 7 ) fba_refund_selling_fees,
				NULL AS fba_fees,
				ROUND( fba_fees * c.percent * c.qty, 7 ) fba_refund_fees,
				ROUND( other * c.percent * c.qty, 7 ) fba_refund_other,
				NULL AS fba_ddp,
				NULL AS adCost,
				NULL AS fba_inventory,
				NULL AS adjustment,
...
SELECT
	...
	ROUND(
		SUM( IFNULL( fbm_sale_amount, 0 ) ) + SUM( IFNULL( fbm_sale_tax, 0 ) ) + SUM( IFNULL( fbm_refund_amount, 0 ) ) + SUM( IFNULL( fbm_sale_selling_fees, 0 ) ) + SUM( IFNULL( fbm_refund_selling_fees, 0 ) ) + SUM( IFNULL( fbm_refund_other, 0 ) ) + SUM( IFNULL( calcuRes, 0 ) ) + SUM( IFNULL( fbm_ddp, 0 ) ) + SUM( IFNULL( fbm_adCost, 0 ) ) + SUM( IFNULL( warehouse_rent, 0 ) ) + SUM( IFNULL( adjustment, 0 ) ) + SUM( IFNULL( liquidation, 0 ) ) + SUM( IFNULL( promotion, 0 ) ) + SUM( IFNULL( shipping_service, 0 ) ) + SUM( IFNULL( lc_adjustment, 0 ) ) + SUM( IFNULL( le_adjustment, 0 ) ) + ROUND( SUM( IFNULL( operation_expenses, 0 ) ) / ( SELECT USD FROM mu_finance_report WHERE id = ' . $report_id . ' ), 2 ) + ROUND( SUM( IFNULL( operation_factory, 0 ) ) / ( SELECT USD FROM mu_finance_report WHERE id = ' . $report_id . ' ), 2 ) + ROUND( SUM( IFNULL( operation_delivery, 0 ) ) / ( SELECT USD FROM mu_finance_report WHERE id = ' . $report_id . ' ), 2 ),
		2 
	) profit,
	ROUND(
		(
	...
	FROM
		...
		FROM
			...
			SELECT
				...
				NULL AS fbm_sale_amount,
				NULL AS fbm_sale_tax,
				ROUND( ( product_sales + shipping_credits + gift_wrap_credits + regulatory_fee + promotional_rebates ) * c.percent * c.qty, 7 ) fbm_refund_amount,
				NULL AS fbm_sale_selling_fees,
				ROUND( selling_fees * c.percent * c.qty, 7 ) fbm_refund_selling_fees,
				ROUND( other * c.percent * c.qty, 7 ) fbm_refund_other,
				NULL AS calcuRes,
				NULL AS fbm_ddp,
				NULL AS warehouse_rent,
				NULL AS adjustment,
